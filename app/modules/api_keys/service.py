...
class ApiKeysService:
    ...
    async def update_key(self, key_id: str, payload: ApiKeyUpdateData) -> ApiKeyData:
        ...
        traffic_class: str | _Unset = (
            _normalize_traffic_class(payload.traffic_class) if payload.traffic_class_set else _UNSET
        )
...
_SUPPORTED_TRAFFIC_CLASSES = frozenset({"foreground", "opportunistic"})
...
def _normalize_traffic_class(value: str | None) -> str:
    normalized = (value or "foreground").strip().lower()
...
def _normalize_traffic_class_lenient(value: str | None) -> str:
    try:
        return _normalize_traffic_class(value)
...
def _to_api_key_data(
    ...
    *,
    ...
    return ApiKeyData(
        ...
        traffic_class=_normalize_traffic_class_lenient(getattr(row, "traffic_class", "foreground")),
