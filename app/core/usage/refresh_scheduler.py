...
class UsageRefreshScheduler:
    ...
    async def _run_loop(self) -> None:
        while not self._stop.is_set():
            ...
            remaining_delay = _remaining_usage_refresh_delay(delay, time.monotonic() - started_at)
...
def _ordered_usage_refresh_accounts(accounts: list[Account]) -> list[Account]:
    return sorted(
        (account for account in accounts if account.status != AccountStatus.DEACTIVATED),
...
def _remaining_usage_refresh_delay(target_delay: float, elapsed_seconds: float) -> float:
    return max(0.0, target_delay - elapsed_seconds)
